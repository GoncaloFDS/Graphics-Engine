...
void main(){
	...
	//if(diffuse_component > 0.0)
		specular_component = pow(max(dot(H, N),0),material.shininess);

	vec3 final_ambient_color = ambient_light * material.ambient;
	vec3 final_diffuse_color = diffuse_light * material.diffuse;
	vec3 final_specular_color = specular_light * material.specular; 
